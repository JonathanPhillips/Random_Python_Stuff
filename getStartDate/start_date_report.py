...
def get_csv_to_dict(url):
    mydict = {}
    with requests.get(url, stream=True) as infile:
        lines = (line.decode('utf-8') for line in infile.iter_lines())
        reader = csv.reader(lines)
        next(reader)
        for row in reader:
            mydict[row[3]] = [row[0] + " " + row[1]]
